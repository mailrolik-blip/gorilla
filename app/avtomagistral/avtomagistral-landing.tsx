...
function LeadForm({ kind }: LeadFormProps) {
  ...
  const formType = isOrder ? 'заказ техники / перевозки' : 'сотрудничество';
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/avtomagistral-leads', {
        method: 'POST',
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit}>
      ...
      <button className={`${styles.btn} ${styles.btnGold}`} type="submit" disabled={status === 'sending'}>
        {status === 'sending' ? 'Отправка...' : isOrder ? 'Заказать обратный звонок' : 'Отправить заявку'}
          ...
          <div className={styles.container}>
            <div className={styles.formsHead}>
              <div>
                ...
                <h2>Две формы для разных сценариев</h2>
              </div>
              <p>
                Первая форма - для клиентов, которым нужна перевозка или спецтехника. Вторая - для владельцев техники,
                транспорта и потенциальных партнеров.
              </p>
            </div>
            <div className={styles.formsGrid}>
              <article className={styles.formArea}>
                ...
                <p>Опишите задачу - мы перезвоним и уточним детали.</p>
                <LeadForm kind="order" />
              </article>
