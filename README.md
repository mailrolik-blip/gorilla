...
Секции «Команды» и «Состав» в /admin переведены в рабочий staff-модуль.

Что сделано:

- в /admin добавлены рабочие staff-секции:
    - Команды
    - Состав
- команды переведены из обзорного режима в рабочий:
    - создание команды
    - редактирование команды
- состав команды переведён в рабочий:
    - добавление участника в состав
    - редактирование записи состава
- список участников для состава теперь загружается из полного /api/participants, а не строится из частичных уже загруженных данных
- для ADMIN и MANAGER доступно:
    - создание и редактирование команд
    - создание и редактирование записей состава
- USER не получает доступ к staff section
- после create/update локальный UI обновляется без ручной перезагрузки
...
Проверка по логике и API:

- ADMIN видит секции команд и состава — ok
- MANAGER видит секции команд и состава — ok
- USER не получает доступ к /admin — ok
- ADMIN может создать и редактировать команду — ok
- MANAGER может создать и редактировать команду — ok
- ADMIN может добавить и редактировать запись состава — ok
- MANAGER может добавить и редактировать запись состава — ok
- /cabinet не сломан — ok
- /admin не сломан — ok

Итог:

- /admin получил ещё один полноценный staff workflow
- после заявок, тренировок и аренды рабочими стали и команды с составами
...
### Обновление пользовательского сценария тренировок
...
Секция тренировок в `/cabinet` переведена из тяжёлого списка в более удобный календарный сценарий.

Что сделано:
- добавлен обзор тренировок по дням;
- реализован сценарий: выбор дня → список тренировок дня → детальный просмотр → запись;
- улучшены empty states и навигация внутри секции;
- сохранён существующий booking flow и API;
- `/admin` и остальные секции кабинета не затронуты.
...
Продолжена работа над безопасностью dev-базы и сценариями восстановления.

Что сделано на текущем этапе:
- начата подготовка backup / restore tooling для PostgreSQL;
- зафиксирован переход от ручных recovery-действий к отдельным скриптам и понятному workflow;
- подготовлен промежуточный этап для дальнейшей сборки recovery-инструментов без риска потерять текущий прогресс.

Что ещё в работе:
- финальные backup / restore scripts;
- удобные команды запуска;
- краткая recovery-документация;
- полная ручная проверка сценария восстановления.
